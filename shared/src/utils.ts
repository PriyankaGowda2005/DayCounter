...
export const createReminder = (offsetMinutes: number, timeOfDay?: string): Reminder => ({
  id: generateId(),
  offsetMinutesFromTarget: offsetMinutes,
  timeOfDay,
});

export const calculateCountdown = (targetDate: string, startDate?: string): CountdownData => {
  const now = new Date();
...
export const formatTimeRemaining = (countdown: CountdownData): string => {
...
export const getUpcomingEvents = (events: Event[], limit: number = 3): Event[] => {
  const now = new Date();
...
export const importFromJSON = (json: string): Event[] => {
  try {
...
export const exportToICS = (events: Event[]): string => {
